...
public final class Ring implements java.io.Serializable {
    ...
    public static double MachineEpsilonFixed = 2.22044604925031300e-16; // least possible value for R64
     ...
     */
    public int MAXCLUSTERMEMORY=1000;
    ...
    public Polynom polynomONE = com.mathpar.polynom.Polynom.polynom_one(NumberZ.ONE) ;      // Type of algebra[0]
   ...
   /**
     ...
     * constant of main algebra
     */
    public Element numberI=com.mathpar.number.Complex.CZ_I;         // Type of algebra[0] or Complex(algebra[0])
    /**
     * constant of main algebra
     */
    public Element numberMINUS_I=com.mathpar.number.Complex.CZ_MINUS_I;;   // Type of algebra[0] or Complex(algebra[0])
  ...
  public Element[] negConst = NumberZ.negConst; 
    ...
    public void createVarPolynoms() {
        ...
        numberZERO = ((flag) && ((zero.numbElementType() & Complex) != Complex)) ? new Complex(zero, zero) : zero;
        numberMINUS_ONE = (flag) ? new Complex(((Complex) numberONE).re.myMinus_one(), ((Complex) numberONE).re.myZero()) : one.myMinus_one();
    ...
    public Ring(Ring ring, int newAlgebra) {
        ...
        System.arraycopy(ring.varLastIndices, 0, this.varLastIndices, 0, ring.varLastIndices.length);
    ...
    public Ring( int algebra, int varNumb) {
        ...
        for (int i = 0; i < varNumb; i++) { 
            ...
            switch(i){
                case 0:varNames[i]="x"; break;
                case 1:varNames[i]="y"; break;
                case 2:varNames[i]="z"; break;
                default:  varNames[i] = "z_" + (i-2);
            }
        }
    } 
    /**
   ...
   public Ring(Ring ring) {
    ...
    public Ring(Ring R, int[] newVar) {   
        ...
        for (int i = 0; i < newVar.length; i++) {
            if (newVar[numbNewVar] != -1) {numbNewVar++; }}// подсчитали сколько их
...
public static Ring cloneWithoutCFormPage(Ring ring) {
    ...
    public static final Ring getRingXYZTofType(int ringType){ return
             new Ring(new int[] {ringType}, new int[] {3}, new String[] {"x", "y", "z", "t"});}

        public static final Ring getRingXYofType(int ringType){ return
    ...
    public static final Ring ringZxyzZuvw
            = new Ring(new int[] {Ring.Z, Ring.Z}, new int[] {2, 5}, new String[] {"x", "y", "z", "u", "v", "w"});
    ...
    */
    public  Ring addVariables(String[] newNames ) {
        Ring my=new Ring();
        int newNumb=newNames.length; int newVarNubm=varNames.length+newNumb;
        try {     
...
//                return "VectorC";
     ...
     */
    public void setConstR() { FuncNumberR fnr= new FuncNumberR(this);
...
//    }
    ...
    public boolean equals(Ring ring) {
        if (varNames.length != ring.varNames.length
                || varLastIndices.length != ring.varLastIndices.length || algebra.length != ring.algebra.length) {
            return false;
        }
    ...
    public boolean isFeild() { int a=(algebra[0] & (Complex-1)); if ((a==Ring.Zp32)||(a==Ring.R64)||(a==Ring.Q)||(a==Ring.R)||(a==Ring.Zp)||(a==Ring.R128))return true; return false;}
